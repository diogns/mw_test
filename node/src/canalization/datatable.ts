...
import { assign, trim, lowerCase, merge, set, clone, isArray, concat, map, each, isNil } from 'lodash';
...
export class DataTableModule {
    ...
    private fetchField(options: IOptions, query: IQuery, column: IColumn, populate: PopulateType): { field: any; populated: boolean } {
        ...
        const field: any = this.schema.path(column.data);
        if (field) {
    ...
    private buildColumnSearchDate(options: IOptions, column: IColumn, search: any): any {
        this.debug(options.logger, 'buildColumnSearchDate:', column.data, search);
        let op: string, from: Date, to: Date;
        if (typeof search.value === 'string' && /^(=|>|>=|<=|<|<>|<=>|><=|>=<)?([0-9.\/-]+)(?:,([0-9.\/-]+))?$/.test(search.value)) {
            op = RegExp.$1;
            const $2 = RegExp.$2;
            ...
            op = search.value.op || '>=<';
            const fromDate = search.value?.from ? new Date(search.value.from) : null;
            if (fromDate && fromDate instanceof Date && !isNaN(fromDate.valueOf())) {
                from = fromDate;
            }
            const toDate = search.value?.to ? new Date(search.value.to) : null;
    ...
    private dataTable(query: IQuery, options: IOptions = {} as IOptions): Promise<IData> {
        this.debug(options.logger, 'query:', util.inspect(query, { depth: null }));
        ...
        return (options.disableCount === true ? Promise.resolve(-1) : this.recordsTotal(options)).then((recordsTotal) => {
            return (options.disableCount === true ? Promise.resolve(-1) : this.recordsFiltered(options, aggregate, recordsTotal)).then(
                (recordsFiltered) => {
