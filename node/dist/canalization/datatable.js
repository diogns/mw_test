...
class DataTableModule {
    ...
    fetchField(options, query, column, populate) {
        ...
        const field = this.schema.path(column.data);
        if (field) {
    ...
    getField(schema, path) {
        let baseField, tail = path, base, indexSep, index = -1, count = 0;
    ...
    buildColumnSearchDate(options, column, search) {
        ...
        if (typeof search.value === 'string' && /^(=|>|>=|<=|<|<>|<=>|><=|>=<)?([0-9.\/-]+)(?:,([0-9.\/-]+))?$/.test(search.value)) {
            ...
            op = search.value.op || '>=<';
            const fromDate = ((_c = search.value) === null || _c === void 0 ? void 0 : _c.from) ? new Date(search.value.from) : null;
            if (fromDate && fromDate instanceof Date && !isNaN(fromDate.valueOf())) {
                from = fromDate;
            }
            const toDate = ((_d = search.value) === null || _d === void 0 ? void 0 : _d.to) ? new Date(search.value.to) : null;
